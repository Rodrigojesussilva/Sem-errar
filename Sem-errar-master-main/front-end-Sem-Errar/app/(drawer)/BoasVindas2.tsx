...
export default function BoasVindas2() {
  ...
  useEffect(() => {
    ...
    const navTimer = setTimeout(() => {
      router.replace('/(drawer)/BoasVindas3'); // ENVIANDO PARA A PRÓXIMA
    }, 3000);
  ...
  return (
    <Animated.View style={[styles.container, { opacity: fadeAnim }]}>
      ...
      <View style={styles.visualArea}>
        {renderOrbit(4, '60deg', 1)}
        {renderOrbit(4, '300deg', -1)}
      </View>
      <View style={styles.content}>
        <Animated.View style={[styles.centralIconWrapper, { transform: [{ scale: pulseAnim }] }]}>
          <MaterialCommunityIcons name="package-variant-closed" size={55} color={COLORS.icon} />
        </Animated.View>
        <Text style={styles.descriptionText}>Tudo que realmente importa, em um só lugar</Text>
      </View>
    </Animated.View>
  );
}
...
const styles = StyleSheet.create({
  ...
  descriptionText: { color: '#777', fontSize: 24, fontWeight: '800', textAlign: 'center', lineHeight: 32 },
});